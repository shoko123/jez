...
class BaseDigModel extends Model implements HasMedia
{
    ...
    //show common to all small finds (About.show() is the only override)
    ...
    public function primaryMedia($item)
    {
        ...
        if (!empty($drawing)) {
            return (object) [
                'hasMedia' => true,
                'fullUrl' => self::$bucketUrl . $drawing->getPath(),
                'tnUrl' => self::$bucketUrl . $drawing->getPath('tn'),
            ];
        } else {
            ...
            if (!empty($photo)) {
                return (object) [
                    'hasMedia' => true,
                    'fullUrl' => self::$bucketUrl . $photo->getPath(),
                    'tnUrl' => self::$bucketUrl . $photo->getPath('tn'),
                ];
            } else {
    ...
    public function allMedia($item)
    {
        ...
        foreach ($drawings as $med) {
            array_push($media->collection, ['fullUrl' => self::$bucketUrl . $med->getPath(), 'tnUrl' =>  self::$bucketUrl . $med->getPath('tn'), 'hasMedia' => true, 'media_id' => $med->id]);
        }
    ...
    public function filler(String $eloquent_model_name)
    {
        return (object) [
            'hasMedia' => false,
            'fullUrl' => self::$bucketUrl . 'app/fillers/' . $eloquent_model_name . '_Filler.jpg',
            'tnUrl' => self::$bucketUrl . 'app/fillers/' . $eloquent_model_name . '_Filler-tn.jpg',
        ];
    }
}
