...
export default {
    ...
    state: {
        ...
        defaultTagCategories: [
            { type: "Stone:Base-Type", mandatory: false, multiple: false, header: "base type", showInFilters: true, showInNewItem: true },
            ...
            { type: "Stone:Vessel-Wall", mandatory: false, multiple: true, header: "Wall", showInFilters: false, showInNewItem: false },
            { type: "Stone:Vessel-Base", mandatory: false, multiple: false, header: "Base", showInFilters: false, showInNewItem: false },

            { type: "Stone:Material", mandatory: true, multiple: false, header: "material", showInFilters: true, showInNewItem: true },

            { type: "Stone:Preservation", mandatory: true, multiple: false, header: "preservation", showInFilters: true, showInNewItem: true },
            { type: "Stone:Life-Stage", mandatory: false, multiple: true, header: "life stage", showInFilters: true, showInNewItem: true },
            { type: "Stone:Morphology", mandatory: false, multiple: true, header: "morphology", showInFilters: true, showInNewItem: true },
            { type: "Stone:Profile", mandatory: false, multiple: true, header: "profile", showInFilters: false, showInNewItem: false },
            { type: "Stone:Production", mandatory: false, multiple: true, header: "production", showInFilters: true, showInNewItem: true },
    ...
    actions: {
        ...
        manageProfileType({ state, commit, rootGetters }, payload) {
            //console.log("manageProfileType");
            //let isFilterNotNewItem = payload.isFilterNotNewItem;
            let show;
            ...
            //show/hide this type as a tab in the appropriate table(filters or newTags - make reactive by using splice)
            let origType = state.defaultTagCategories.find(x => x.type === "Stone:Profile");
            let newType = { ...origType };
            ...
            if (payload.isFilterNotNewItem) {
                newType.showInFilters = show && payload.actionIsSelect;
            } else {
                newType.showInNewItem = show && payload.actionIsSelect;
            }
            commit("tag/modifyType", newType, { root: true });
        ...
        //called when base-type active-or-passive is selected.
        managePreservationType({ state, commit, rootGetters, dispatch }, payload) {
            //clear tags of type Stone:Preservation
            let tagsToUnSelect = rootGetters[`tag/tags`]
                .filter(x => ((payload.isFilterNotNewItem && x.selectedInFilter) || (!payload.isFilterNotNewItem && x.selectedInNewItem)))
                .filter(y => y.type === "Stone:Preservation");
            ...
            let tag = rootGetters[`tag/tags`].find(y => y.type === "Stone:Preservation" && y.name === "Fragment");
            ...
            commit("tag/modifyTag", tagModifyRequest, { root: true });
        },
