<template>
  <v-container fluid>
    ...
    <v-row class="ga-1">
      <v-col cols="2">
        <v-select v-model="nf.clean" label="Clean" item-title="text" item-value="text"
          :items="cleanInfo.options"></v-select>
      </v-col>
    </v-row>
...
import { TFields, TFieldInfo, TFieldsErrors, TFieldsDefaultsAndRules } from '@/types/moduleTypes'
...
console.log(`Locus(${isCreate.value ? 'Create' : 'Update'}) dataNew: ${JSON.stringify(dataNew.value, null, 2)}`)
// setup - end
...
const locusFieldsWithOptions = computed(() => {
  return fieldsWithOptions.value as Partial<Record<keyof TFields<'Locus'>, TFieldInfo>>
})

const cleanInfo = computed(() => {
  return locusFieldsWithOptions.value['clean']!
})
// Lookup/Enum fields - end
