...
export async function parseUrlQuery(qp: LocationQuery) {
  ...
  for (const [key, value] of Object.entries(qp)) {
    ...
    switch (group.code) {
      ...
      case 'SF':
        {
          const res = await processUrlFS(group, optionTexts, selectedFilters)
          if (!res.success) {
            return res
          }
        }
...
async function processUrlFS(group: TGroup, texts: string[], filterAllOptionKeys: string[]) {
  const { trio } = storeToRefs(useTrioStore())
  // Each search group has max 6 items
  if (texts.length > 6) {
    return {
      success: false,
  ...
  // Empty the group's search texts
  group.optionKeys.forEach((key) => {
    trio.value.optionsObj[key]!.text = ''
  })

  // Push search texts to options, and optionKeys to filterAllOptionKeys.
  texts.forEach((text, index) => {
    const optionKey = group.optionKeys[index]!
    trio.value.optionsObj[optionKey]!.text = text
    filterAllOptionKeys.push(optionKey)
  })
  return { success: true }
}
