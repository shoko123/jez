...
export default {
  ...
  created() {
    ...
    if (!this.$store.getters["pkr/fromDbAreasSeasons"]) {
      console.log("PickerExisting. loading areas...");
  ...
  methods: {
    ...
    goTo() {
      if (!this.$store.getters["pkr/item"]) {
        ...
        this.dialog = false;
        this.$router.push({
          path: `${this.$store.getters["mgr/status"].moduleAppBaseUrl}/${this.$store.getters["pkr/item"].id}/show`
        });
      }
    },
