...
export default {
  ...
  computed: {
    ...
    areasFormatted() {
      return this.areas.map(ar => ({ tag: ar.year + "." + ar.area, year: ar.year, name: ar.area, id: ar.id}));
...
selectedArea() {
    ...
    OnCloseSelector() {
      ...
      console.log("selected area: " + this.areaId);
      


      console.log(this.$store.getters.getAreaById(this.areaId));
      console.log("selected area: " + this.selectedArea().year + '.' + this.selectedArea().area + ' id => ' + this.selectedArea().id);
    },
  },
